...
- Update `docs/OPERATIONS.md` when configuration, verification, publishing, diagnostics, or recovery changes.
- Create an ADR in `docs/decisions/` when relevant alternatives exist and the decision is not evident from the code.
- Never include credentials, `.env` values, personal data, generated outputs, or sandbox artifacts in versioned documentation.
